let sliderItem = document.querySelectorAll('.photoItem');
let i = 1
setInterval(() => {
    sliderItem.forEach(e => {
        e.classList.remove('active')
    });
    sliderItem[i].classList.toggle('active');
    i == sliderItem.length - 1 ? i=0 : i++

}, 5000);
/////////////////////

let feutureLists = document.querySelectorAll('.feutureLists ul li');
feutureLists.forEach(e => {
    let i = e.innerHTML;
    e.innerHTML=`<svg class='star' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
